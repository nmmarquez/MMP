...
Type objective_function<Type>::operator() ()
{
    ...
    PARAMETER_VECTOR(sigmas); // 2 one for each alternative group
    ...
    Sigma(0,0) = pow(exp(sigmas)[0], 2.);
    Sigma(1,1) = pow(exp(sigmas)[1], 2.);
    Sigma(1,0) = exp(sigmas)[1] * exp(sigmas)[0] * rho;
    Sigma(0,1) = exp(sigmas)[1] * exp(sigmas)[0] * rho;
